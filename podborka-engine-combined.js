...
(function () {
  ...
  function renderList() {
    ...
    listEl.querySelectorAll('.podborka-row-remove').forEach(b => {
      b.onclick = () => { const s = loadState(); s.items.splice(Number(b.dataset.idx), 1); saveState(s); };
    });
  ...
  async function downloadAllPresentations(withContact) {
    ...
    try {
      ...
      for (let ci = 0; ci < chunks.length; ci++) {
        ...
        const baseName = useContact ? 'Подборка' : 'Подборка_без_цены';
